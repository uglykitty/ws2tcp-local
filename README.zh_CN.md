...
如果本机只能通过另一个代理服务器访问网络（例如在公司网络中），可用
`--upstream-proxy` 指定：
...
由代理解析域名）、`socks5://`（先在本地解析域名）。`user:pass@` 认证信息
可选，其中的特殊字符需要百分号编码。端口默认为：`http://` 80，SOCKS5 协议 1080。

所有出口连接都会经过它：访问 gateway 的隧道、启动时的健康检查和 token 登录，
被路由规则判定为直连的请求（由代理代为连接），以及规则列表的下载。设置后不会读取
