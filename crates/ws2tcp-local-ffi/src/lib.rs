...
use ws2tcp_local::{ProxyMode, Settings, run_proxy};
...
pub unsafe extern "C" fn ws2tcp_init_logging(log_level: *const c_char) -> i32 {
    ...
    match ws2tcp_local::init_logging(log_level.as_deref()) {
        Ok(()) => WS2TCP_OK,
...
fn parse_settings(json: &str) -> Result<Settings> {
    ...
    let buffer_size = settings
        .buffer_size
        .unwrap_or(ws2tcp_local::cli::DEFAULT_BUFFER_SIZE);
    if buffer_size == 0 {
        bail!("buffer_size must be greater than 0");
    }
    let rule_refresh_interval_secs = settings
        .rule_refresh_interval_secs
        .unwrap_or(ws2tcp_local::cli::DEFAULT_RULE_REFRESH_INTERVAL_SECS);
    ...
    Ok(Settings {
        listen: settings.listen.unwrap_or(
            ws2tcp_local::cli::DEFAULT_LISTEN
                .parse()
...
mod tests {
    ...
    fn parses_minimal_settings_with_defaults() {
        ...
        assert_eq!(settings.buffer_size, ws2tcp_local::cli::DEFAULT_BUFFER_SIZE);
